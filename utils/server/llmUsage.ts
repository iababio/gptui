...
import { LlmsDb, UserDb, getDb } from './storage';

export async function verifyUserLlmUsage(
  userId: string,
  modelId: OpenAIModelID,
) {
  if (!(await checkUserUSDConsumptionMonthly(userId)))
    throw new Error(
      "Uh-oh! You've reached the monthly API limit. Please reach out to the admin team for assistance.",
    );
}

export async function checkUserUSDConsumptionMonthly(
  userId: string,
): Promise<boolean> {
  ...
  const totalPriceUSDUsage = modelUsage.reduce(
    (prev, curr: UserLlmUsage) => prev + (curr.totalPriceUSD || 0),
    0,
  );

  const currentUserLimitUSD =
    currentUser.monthlyUSDConsumptionLimit ?? DEFAULT_USER_LIMIT_USD_MONTHLY;
...
export async function saveLlmUsage(
  ...
  if (modelPriceRate1000) {
    modelUsage.totalPriceUSD =
      (tokens.prompt / 1000) * modelPriceRate1000.promptPriceUSDPer1000 +
